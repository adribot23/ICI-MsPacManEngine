...
public class Ghosts extends GhostController {
	...
	public EnumMap<GHOST, MOVE> getMove(Game game, long timeDue) {
        ...
        // 1. Calcular la power pill más lejana a Pac-Man
        int farthestPP = -1;
        int maxDist = -1;
        ...
        // 3. Asignar fantasmas a esquinas según cercanía
        GHOST[] ghosts = GHOST.values();
        boolean[] assigned = new boolean[4];
        int[] ghostToTarget = new int[4]; // Índice de power pill objetivo para cada fantasma (-1 si no asignado)
        for (int i = 0; i < 4; i++) ghostToTarget[i] = -1;

        // Contar cuántos fantasmas comestibles hay
        int edibleCount = 0;
        for (int i = 0; i < 4; i++) {
        ...
        if (edibleCount >= 2) {
            // Asignar los dos fantasmas comestibles más cercanos a la esquina más lejana
